...
# The node where the list starts is called the head of the blist and the node where it ends, or last node, is called the tail of the list.
...
# If the list isn't empty, then we will make the 'next' pointer of the last node(pointed at by 'tail') point to the new node instead of None
...
# If the postition is somewhere in between, then we create a temporary node which traverses the list up to the previous position of the position we want to enter the new node
...
# (If the value is found multiple times, only the first occurence of the value will be deleted.)
