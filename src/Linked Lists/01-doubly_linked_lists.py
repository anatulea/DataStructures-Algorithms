...
class DoublyLinkedList():
    ...
    def prepend(self, data):
        ...
        if self.head == None:
            ...
            # We establish a two-way link by making the previous of the present head point to the new node
    ...
    def insert(self, position, data):
        if position == 0:
            # Inserting at position 0 is equivalent to prepending. So instead of repeating code, we simple call the prepend method
            self.prepend(data)
            return
        if position >= self.length:
            # Similarly, inserting at position >= the length of the list is equivalent to appending, so we call the append method
            self.append(data)
            return
        else:
            new_node = Node(data)
            current_node = self.head
            # We traverse upto one position before the position where we want to insert the new node
            for i in range(position - 1):
                current_node = current_node.next
            # We make the previous of the new node point to the current node
            new_node.previous = current_node
            # And the next point to the next of the current node.
            new_node.next = current_node.next
            # Then we break the link between the current node and the next node and make the next of the current node point to the new node
            current_node.next = new_node
            # And finally we update the previous of the next node to point to the new node instead of the current node. This way, the new node gets inserted in betwwen the current and the next nodes.
    ...
    def delete_by_value(self, data):
        ...
        if current_node.data == data:
            ...
            # If after deleting the first node the list becomes empty or there remains only one node, we set the tail equal to the head
        ...
        if current_node != None:
            ...
            # If the node deleted is not the last node(i.e., the node next to the next to the current node is !- None),
            if current_node.next != None:
                # Then we set the previous of the node next to the deleted node equal to the current node, so a two-way link is established
